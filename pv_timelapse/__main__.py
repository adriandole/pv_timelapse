...
parser.add_argument('-c', '--codec', default='h265', type=str, dest='c',
                    help='Video codec to encode with', choices=['h265', 'h264'])
parser.add_argument('-q', '--quality', default=23, type=int, dest='q', choices=range(0,52),
                    help='Amount of compression; higher = worse quality. 18-28 recommended.')
parser.add_argument('-ef', '--efficiency', default=8, type=int, dest='ef', choices=range(0,9),
                    help='Compression efficiency. No effect on quality. Higher efficiency'
                         ' = longer encoding time.')
parser.add_argument('--custom_ffmpeg', type=dict, default=None, dest='custom',
                    help='Custom output parameters for FFMPEG. Not recommended.')
...
parser.add_argument('-w', '--windows', action='store_true', dest='w',
                    help='Preset for maximum compatibility with Windows Media Player')
...
if in_args.custom is not None:
    ...
    output_dict = {'-codec:v': codecs[in_args.c], '-preset': efficiency[in_args.ef],
                   '-crf': str(in_args.q)}
...
try:
    p = Params(source_path, write_path, start_datetime, end_datetime, in_args.d,
               in_args.r, in_args.fdn, in_args.imn, in_args.lt, input_dict, output_dict)
    create_timelapse(p)
